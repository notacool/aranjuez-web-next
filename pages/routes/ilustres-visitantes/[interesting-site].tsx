...
import InterestingSitesDetailsForVipPeople from "@components/InterestingSiteDetails/InterestingSitesDetailsForVIPPeople";
import InterestingSitesDetailsForVipPeopleMobile from "@components/InterestingSiteDetails/InterestingSitesDetailsForVIPPeopleMobile";
...
export default function InterestingSiteView() {
  ...
  useEffect(() => {
    const selectSite = () => {
      const site = INTERESTING_SITES.filter(
        (site) => dinamicURL.split("/")[3] === site.url
      );
  ...
  return (!isDesktop && !isMobile && !isTablet) || !currentSite ? (
    ...
    <>
      <NavBarDesktop />
      <InterestingSitesDetailsForVipPeople url={currentSite.url} />
    </>
  ) : (
    <>
      <NavBarMobile open={false} />
      <InterestingSitesDetailsForVipPeopleMobile url={currentSite.url} />
