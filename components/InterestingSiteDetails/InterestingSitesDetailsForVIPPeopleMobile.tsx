...
import { Box, Button, Container, Grid2, IconButton, List } from "@mui/material";
...
import VipPeopleForInterestingSites from "./VipPeopleForInterestingSites";
import { ILLUSTRIOUS_PEOPLE } from "@const/illustriousPeople";
import VipPeopleForInterestingSitesMobile from "./VipPeopleForInterestingSitesMobile";
...
export default function InterestingSitesDetailsForVipPeopleMobile({
  url,
}: RouteProps) {
  ...
  const filterVipPeople = ILLUSTRIOUS_PEOPLE.filter(
    (artist) => artist.ubication === filterSite[0].name
  );
        ...
        >
          ...
          <Grid2 size={{ xs: 12 }} sx={{ marginTop: 2 }}>
            {filterVipPeople.length === 0 ? (
              ...
              <VipPeopleForInterestingSitesMobile name={filterSite[0].name} />
            )}
          </Grid2>
