import { Route } from "types";

export const ROUTES_INFO: Route[] = [
  {
    routeId: 1,
    url: "ciudad-cortesana",
    title:
      "Ruta de la ciudad cortesana",
    imageUrl:
      "https://minio.notacool.com/aranjuez/interestingSites/web/casa%20de%20infantes/aranjuez_casa_de_infantes_4.jpg",
    videoUrl:
      "https://minio.notacool.com/aranjuez/Ruta de la ciudad cortesana.mp4",
    to: "1",
    introduction: [
      "En 1750 Santiago Bonavía trazó el Plan ordenador de una nueva ciudad. Esta ciudad surgía por las nuevas necesidades cortesanas de dar cabida tanto a los visitantes que afluían a las Jornadas de Primavera -periodo en que la Corte se asentaba en Aranjuez- como a dotar de servicios fundamentales tanto a estos visitantes como a los empleados encargados del mantenimiento del Real Sitio.",
      "Desde los tiempos de Felipe II se había prohibido el asentamiento de población, a fin de garantizar el uso exclusivo del lugar para el ocio y descanso de la Familia Real.",
      "El plan de Bonavía contemplaba una ciudad de desarrollo Este Oeste, limitada al Norte por el río Tajo y unida al núcleo palatino por la Plaza de San Antonio, espacio bisagra entre la residencia real y la zona de habitación del resto de la población.",
    ],
    videoDescription: [
      "La zona palatina está integrada por el palacio, los jardines a su alrededor, la plaza para los espectáculos (Plaza de Parejas) y las casas de Oficios, Caballeros y Fogones. Este núcleo se conformó entre los siglos XVI y XVII.",
      "Al otro lado del palacio, Santiago Bonavía formó un tridente, inspirado en el urbanismo renacentista italiano, en el que se construyeron los primeros palacios para nobles y altos funcionarios al servicio de la Casa Real. Este tridente está integrado por la preexistente calle de la Reina, conformada en tiempos de Carlos V y las del Príncipe e Infantas. El tridente parte de la amplia zona diáfana que organizaba la entrada a la ciudad a través del Puente de Barcas sobre el río Tajo, desde el nuevo camino de acceso procedente del Puente Largo sobre el Jarama.",
      "El primer recinto de Santiago Bonavía estaba, a la manera barroca, ceñido por un amplio paseo arbolado de traza rectangular, limitado por las calles del Foso, Abastos y Florida.",
    ],
    description: [
      "El éxito de las convocatorias anuales durante el reinado de Carlos III, obligó a ampliar el trazado inicial y transformar el espacio acotado en uno abierto a ulteriores desarrollos. De este modo, de la mano de Jaime Marquet y posteriormente de Juan de Villanueva, la retícula incipiente al norte se amplió con una trama de calles reticular, inspirada en los modelos de las ciudades de la Antigüedad, el conocido como trazado hipodámico o en damero. Así surgieron hacia el Sur paralelas a la calle de San Antonio y se prolongaron las transversales dirección Norte Sur que llegaron hasta la falda de las estribaciones de la Mesa de Ocaña. Una de ellas, la Carrera de Andalucía constituía el paso del nuevo camino de Madrid a Andalucía promocionado por Carlos III.",
      "Por el damero se distribuyeron plazas así como distintos tipos de vivienda, todas siguiendo la modulación de la manzana, crujías alrededor de un amplio patio central. De este modo, al exterior, apenas se diferencian los palacios, las casas de jornada o alquiler para visitantes y las corralas, viviendas populares que podían alcanzar mayor altura.",
      "El espíritu ilustrado, las nuevas ideas de higiene y ciudad moderna hicieron que estas calles fueran orientadas para propiciar la ventilación de la ciudad con los vientos de la sierra de Guadarrama, y se dotase progresivamente de arquitecturas que respondían a todo tipo de servicios: hospital, teatro, cuarteles, plaza de toros, casas de gobierno y administración, casas de abastos, almacenes, fondas, paradores y hoteles, restaurantes, tiendas de artículos de lujo y repostería (de ahí el nombre de la calle Almíbar), mercado, cementerio y estación de ferrocarril.",
      "El aspecto de la ciudad cortesana, de inspiración clásica, funcional y homogéneo, apenas deja traslucir las distintas fases en que se conformó a lo largo de los siglos XVIII y XIX.",
    ],
    mapUrl:
      "https://www.google.com/maps/d/u/1/embed?mid=1EgRMgAxupyrBko-u5yz_VOMmWS6PxPM&ehbc=2E312F",
      
  },
  {
    routeId: 2,
    url: "palacios-templos",
    title: "Ruta de los palacios y templos",
    imageUrl:
      "https://minio.notacool.com/aranjuez/interestingSites/web/palacio%20de%20godoy/aranjuez_palacio_de_godoy_2.jpg",
    videoUrl:
      "https://minio.notacool.com/aranjuez/ruta de los palacios y los templos_2.mp4",
    to: "2",
    introduction: [
      "Bajo el referente jerárquico de poder y representación de la casa del rey - el Palacio Real-, otros palacios de menor entidad afloraron en la ciudad. Pertenecían a miembros de la Familia Real, nobles y altos cargos o funcionarios de la Corona. Estos edificios se adaptaron a las formas de las manzanas y las sucesivas normas de construcción de Santiago Bonavía, Jaime Marquet y Juan de Villanueva (también autores de algunos de los ejemplos más destacados), que fijaban las alturas, la organización de los vanos en fachada, las alineaciones y los materiales. Todos tuvieron otra característica común, la indiferenciación formal: los palacios no pudieron ostentar las armas o escudos de sus propietarios, que de ninguna manera podían competir con los escudos de la Corona, la propietaria del Real Sitio. De ahí que estas arquitecturas no presenten rasgos diferenciales, salvo algunas portadas más elaboradas.",
    ],
    videoDescription: [
      "Distinto fue el criterio para las arquitecturas decimonónicas que se construyeron al Suroeste del Palacio Real, levantadas durante el reinado de Isabel II y ocupado por un nuevo perfil de clases privilegiadas: la nueva nobleza y la alta burguesía de la capital. Han pervivido el palacio de Baviera y el palacio de El Deleite. Son muestra de los movimientos eclécticos arquitectónicos del siglo XIXI, de absoluta libertad constructiva.",
    ],
    description: [
      "Otro capítulo es el de los edificios religiosos. No son muy abundantes, pero en su breve nómina, cada uno de ellos responde a una distinta función y tipología, siempre bajo el patrocinio real. La iglesia de San Antonio, como Capilla Real; San Pascual, un convento promocionado por Carlos III e Isabel II; la iglesia de Alpajés, la parroquia de un antiguo núcleo medieval de población, de la que fueron cofrades y benefactores, a partir de Carlos II, los reyes españoles.",
    ],
    mapUrl:
      "https://www.google.com/maps/d/embed?mid=16keAW1TvU26s_dQiUfG-CQx_ji_Ipxk&ehbc=2E312F",
  },
  {
    routeId: 3,
    url: "agua",
    title:
      "Ruta del agua y las Colonias agropecuarias",
    imageUrl:
      "https://minio.notacool.com/aranjuez/interestingSites/web/las%20doce%20calles/aranjuez_las_doce_calles_2.jpg",
    videoUrl: "https://minio.notacool.com/aranjuez/Ruta del agua_2.mp4",
    to: "3",
    introduction: [
      "El Paisaje Cultural de Aranjuez no hubiese sido posible sin la presencia de los ríos Tajo y Jarama. Abastecer de agua potable a los residentes, regar el conjunto de jardines, huertas, calles y plazas arboladas, controlar las crecidas de los ríos y establecer una red de comunicaciones que permitiese el paso sobre los cauces, no hubiese sido posible sin una infraestructura ingenieril hidráulica y de obras públicas. El conjunto de Ingenios Hidráulicos está considerado el más completo e innovador de la Edad Moderna en la Península Ibérica.",
    ],
    videoDescription: [
      "El eje lo constituye la Presa del Embocador, al Este de la zona urbana, y los dos canales de riego que de ella parten, abrazando la amplia superficie vegetal del valle, que contrasta con las áridas elevaciones de la meseta castellana en que se asienta. Estos canales son el de las Aves o Sotomayor y el de El Embocador o la Azuda que ramificados se distribuyen por todo el territorio. A ellos se sumó la acequia de Colmenar. El agua posibilitó la traza de calles y plazas arboladas y la creación de huertas en el espacio acotado por los ríos Tajo y Jarama. La calle de la Reina, Picotajo y las Doce Calles, del siglo XVI, son la muestra más antigua de ordenación paisajística a gran escala en el mundo.",
    ],
    description: [
      "El riego de los jardines se complementó con el embalse y presa del Mar de Ontígola, obra renacentista que marcó un hito en la historia de la ingeniería y que sirvió para suministrar el agua a las fuentes del jardín de la Isla.",
      "A lo largo del río Tajo se dispusieron en el siglo XVI los puentes de la Reina, de Barcas y la presa de Palacio, continuamente reconstruidos a lo largo del tiempo por las riadas.",
      "El siglo XVIII supuso una fase también innovadora. En primer lugar, se dotó de agua potable a la ciudad, mediante un sistema de aprovisionamiento a partir de manantiales cercanos a Ocaña. En segundo lugar, se proyectaron ciudades rústicas, complementarias a la ciudad cortesana. Siguiendo los principios fisiocráticos del siglo de la Ilustración, estas colonias dieron respuesta al interés regio por la investigación y la experimentación científicas agropecuarias. Dispersas por el territorio, aunque conectadas con la ciudad, pueden citarse el Real Cortijo de San Isidro, la Casa de la Monta/Sotomayor, El Deleite, la Flamenca, las Infantas y Villamejor.",
    ],
    mapUrl:
      "https://www.google.com/maps/d/embed?mid=1rqz1wA21PUPz8j5MNILM9IUiM3Twpnk&ehbc=2E312F",
  },
  {
    routeId: 4,
    url: "miradores",
    title: "Ruta de los miradores",
    imageUrl:
      "https://minio.notacool.com/aranjuez/interestingSites/web/puente%20de%20barcas/aranjuez_puente_de_barcas_1.jpg",
    videoUrl:
      "https://minio.notacool.com/aranjuez/Ruta de los miradores.mp4",
    to: "4",
    introduction: [
      "El trazado de las calles y plazas de las huertas, de los jardines y de la ciudad de Aranjuez, regido por las perspectivas y la geometría, así como los contrastes entre el río Tajo, las masas de arbolado y la arquitectura de la ciudad tienen una vista especial desde ciertos puntos, algunos elevados en las colinas que rodean el valle, otros a las orillas del río.",
    ],
    videoDescription: ["Casi todos estos miradores fueron concebidos en su tiempo expresamente para el deleite y la tranquilidad de la contemplación del paisaje y recibieron nombres tan sugerentes como Altos de Mira el Rey o Monte Parnaso."],
    description: ["Estas vistas resultan imprescindibles para comprender el alcance territorial del Paisaje Cultural de Aranjuez, reconocido como Patrimonio Mundial por la UNESCO en 2001."],
    mapUrl:
      "https://www.google.com/maps/d/embed?mid=182Ulj-bBqW57uaLNnAngbDW1ZSSWXvs&ehbc=2E312F",
  },
  {
    routeId: 5,
    url: "ilustres-visitantes",
    title:
      "Ruta de los personajes ilustres",
    imageUrl:
      "https://minio.notacool.com/aranjuez/interestingSites/web/plaza%20de%20santiago%20rusi%C3%B1ol/aranjuez_plaza_santiago_rusinol_1.jpg",
    videoUrl:
      "https://minio.notacool.com/aranjuez/ruta de los ilustres visitantes.mp4",
    to: "5",
    introduction: [
      "Lugares tanto del paisaje cultural, las vegas del Tajo y el Jarama, las calles arboladas, las huertas, también mas recientes como las vías del la segunda línea de ferrocarril de España (1851) propiciaron los elogios de personajes ilustres que visitaron Aranjuez.",
    ],
    videoDescription: [
      "Ya conformada la ciudad ilustrada en el siglo XVIII, ésta y sus edificios fueron testigos de los pasos de conocidos y afamados escritores, pintores, músicos, directores de cine, actores, actrices, toreros, políticos o diplomáticos, españoles y extranjeros.",
      "Estas figuras dejaron testimonios de su experiencia de ocio y deleite en Aranjuez, bien obras literarias, pinturas o documentos.",
    ],
    description: [
      "En la ruta podremos encontrar a los escritores Miguel de Cervantes, Calderón de la Barca, María Rosa de Gálvez; diplomáticos, gobernantes, políticos, como Cosme de Medicci, el Duque de Saint Simon; eruditos como Antonio Ponz; a los pintores Velázquez o Santiago Rusiñol; las grandes figuras de la música Farinelli y Joaquín Rodrigo; actores, actrices y directores de cine del renombre de Orson Welles, Sofía Loren y Ava Gardner.",
      "Esta ruta invita a interpretar el Paisaje Cultural de Aranjuez, siguiendo sus pasos, con la mirada y las palabras de quienes nos antecedieron.",
    ],
    mapUrl:
      "https://www.google.com/maps/d/embed?mid=1RGMQ8D4gPI-nfaetOmoAJ6Y-KVaB_lQ&ehbc=2E312F",
  },
 {
    routeId: 6,
    url: "picotajo-doce-calles",
    title:
      "Ruta de huertas de Picotajo y las Doce Calles",
    imageUrl:
      "",
    videoUrl:
      "",
    to: "6",
    introduction: [
      "Las calles, plazas arboladas y huertas de Picotajo, constituyen el primer ejemplo mundial de jardinería a gran escala, cien años anteriores a los jardines de Versalles, a los que influyó con la geometría y perspectivas de sus calles arboladas y plazas en forma de sol, como las Doce Calles (2 Rutas R3 R4 R5). Su conformación y antigüedad (la primera calle se trazó en 1553) fueron determinantes para la inscripción del paisaje cultural de Aranjuez en la Lista de Patrimonio Mundial en 2001.",
    ],
    videoDescription: [
      "",
      ],
    description: [
      "Por este amplio espacio discurría el acceso de la corte a Aranjuez procedente de Madrid. Durante el periodo de los Austrias (siglos XVI y XVII) la entrada a Aranjuez se realizaba por el puente que existió sobre el río Jarama en el punto de la puerta de Legamarejo (46), para pasar a la calle de Entrepuentes (47) y alcanzar la plaza frente al jardín de la Isla (48). Aquí a través de un puente desaparecido sobre el río Tajo (48), se cruzaba el jardín hasta el puente de la Ría (56, Ruta 7) para llegar por la calle de Madrid (55, Ruta 7) hasta el Palacio Real.",
      "A partir del siglo XVIII, con el advenimiento de los Borbones, el acceso a Aranjuez pasaría a tener otra trayectoria. La construcción del Puente Largo y con ello la creación de un nuevo tramo final del camino desde Madrid a Aranjuez, que enlazaba este puente con la ciudad recién fundada por Fernando VI.  Durante este reinado de Fernando VI se dispone al Norte la plaza conocida como Puerta Cirigata (50), evocando en su perímetro de pies derechos y cerca el diseño de las Doce Calles (2 Rutas R3 R4 R5). En la Puerta Cirigata, también conocida como Pical del Rey, se desdobla el camino que viene de Madrid por el Puente Largo sobre el río Jarama: por un lado, hacia el Puente de Barcas (10. Rutas R1 R3 R4 R5), la entrada norte a la ciudad para los pobladores; por otro, para el rey y los cortesanos, a través de las huertas, por la calle del Rey, hasta el Raso de la Estrella (58, Ruta 7) y el Palacio Real.",
      "El camino que seguía la comitiva real discurría, así, desde la puerta Cirigata (50) hasta la plaza frente el jardín de la Isla (48). En este punto, en vez de cruzar el jardín de la isla, la comitiva se dirigía hasta el puente Verde sobre el río Tajo (58, Ruta 7) y continuar, en la otra orilla, hasta la plaza de los Coroneles (59, Ruta 7), la calle de Coroneles (53, Ruta 7) y alcanzar la residencia real.",
    ],
    mapUrl:
      "",
  },
  {
    routeId: 7,
    url: "raso-de-la-estrella",
    title:
      "Ruta del Raso de la Estrella",
    imageUrl:
      "",
    videoUrl:
      "",
    to: "7",
    introduction: [
      "El Raso de la Estrella (40, Rutas R1 R4 R5), debe su nombre a un desaparecido asentamiento medieval. Este amplio espacio al Oeste del palacio, considerado la entrada oficial al Aranjuez, conoció diversas etapas en su conformación, adaptadas éstas a las necesidades protocolarias de los Austrias y los Borbones.",
    ],
    videoDescription: [
      "",
      ],
    description: [
      "La configuración del siglo XVI, acpometida por Juan Bautista de Toledo quedó terminada en 1563. Era un recinto poligonal limitado por los caminos de Toledo (51) y de Madrid (55) que antecedía al Palacio Real. Estas dos calles se unían en su extremo más lejano del palacio por dos calles, que cerraban el polígono. La primera, de la Escuadra, llamada así por estar en ángulo con la de la Huelga, y esta segunda, con un nombre que hace referencia a su uso primigenio como zona de pasto de ganado y posteriormente de expansión y ocio.  Todas las calles se conformaban con dobles hileras de árboles a cada lado según la tradición flamenca.",
      "Para la diafanidad de este amplio espacio y la adecuación a la estética urbana renacentista. se había ordenado en 1561 el derribo de algunas construcciones. Considerando el punto de fuga el palacio, se aplicaron los principios geométrivos y perspectivos de la estética regia, al servicio de la imagen del poder.",
      "La imagen es la que se puede contemplar en la vista de L’Hermitte, el óleo del Museo del Prado y los grabados anteriores a la ampliación del palacio del siglo XVIII, un polígono de calles de doble hilera de árboles.",
      "Frente a la residencia real se dispuso como línea de separación una calle paralela a la fachada, verdadero telón de arbolado. La calle de Madrid (55), formaba parte del itinerario que venía desde la capital a través de Picotajo (1 Ruta 6), y cruzaba el puente desaparecido sobre el río Tajo  (48 Ruta 6) y el puente de la Isla (57).",
      "En el siglo XVIII el polígono se transformó en un pentadente, proyecto de Santiago Bonavía, quien añadió tres calles entre las de Toledo y Madrid. Sabatini lo terminó, articulando la forma dela parte Oeste y disponiendo al Este,  en el punto de convergencia de las cinco calles, un espacio elíptico que antecedía al patio de armas de la residencia de los reyes.",
      "La nueva traza favorecía la imagen protocolaria de los Borbones, apoyada en la fuerza militar, como símbolo de poder. De este modo se dispusieron los simétricos cuarteles de Guardias Españolas y Walonas, levantados por Jaime Marquet, a ambos lados de la calle Coroneles (53). Destacan las calles centrales de Guardias Walonas y Españolas (52, 54) y la plaza de Coroneles (59, G Ruta 4). Los pasos que comunicaban con las huertas de Picotajo (1, Ruta 6) y cruzaban el río, como el Puente de la Isla (57) y los miradores del Puente Verde (58, G Ruta 4, 49 Ruta 6) (H) y de la Ría (56, I Ruta 4) ofrecen, al norte, el fondo perspectivo del Tajo, de las calles de Picotajo y del jardín de la Isla.",
      "En el siglo XIX, la llegada del ferrocarril introdujo novedades en el Raso de la Estrella. Esta línea Madrid- Aranjuez, la segunda línea de la Península, fue inaugurada en 1851. Se ubican en el Raso de la Estrella los vestigios más antiguos del patrimonio arqueológico ferroviario peninsular, habida cuenta de que los del trazado Barcelona-Mataró desaparecieron.",
      "Las vías atravesaron la superficie hasta situar la primitiva estación frente al palacio real (60). Era la nueva entrada a Aranjuez, por el tradicional espacio, adaptado a la modernidad de la era industrial.  En los años XX se construyó la nueva estación (40).",
      "Hoy en día, el Raso de la Estrella está bajo un plan director para su puesta en valor patrimonial, como la Entrada Real a Aranjuez. El primero de los proyectos ha sido la recuperación del trazado original de la calle de Guardias Walonas (52) y la salvaguarda de la playa de vías del siglo XIX y primeros años del XX.",
    ],
    mapUrl:
      "",
  },
];
